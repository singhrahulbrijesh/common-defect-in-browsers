...
sub process_git_repo ($$) {
	...
	my $insert_dag = $dbh->prepare(q{insert into git_dag(child, parent, ordering) values (?,?,?)});
	...
	my @history = split(/\n/, qx(git log --full-history --numstat -M -C --pretty=format:"-prxh-START_LOG_MSG-%n%H%n%T%n%an <%ae>%n%aD%n%cn <%ce>%n%cD%n%P%n%d%n%s%n%b%n-prxh-END_LOG_MSG-%n")); 
	#testermy @history = split(/\n/, qx(git log -1 --numstat -M -C --pretty=format:"-prxh-START_LOG_MSG-%n%H%n%T%n%an <%ae>%n%aD%n%cn <%ce>%n%cD%n%P%n%d%n%s%n%b%n-prxh-END_LOG_MSG-%n" 45ba9dd2007da23da5ac21179451c3c9fee30a96)); 
	
	
#	Use this definition of @history to check logs and to evaluate the differences between multiple mirrors
#	my @history;
#	if (!-e 'prxh-logs.out') {
#		print STDERR "No log for git repo $path\n";
#		return;
#	}
#
#	my $file = IO::File->new('prxh-logs.out', 'r');
#	while(<$file>) {
#		push @history, $_;	
#	}
#	$file->close;
